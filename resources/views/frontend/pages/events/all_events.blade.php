...
@section('title', 'Events - Bengali Hindu Unity')
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-lg-8" id="events-container">
                @include('frontend.pages.events.partials.events_grid')
            </div>
            ...
            <div class="col-lg-4">
                ...
                <div class="sidebar-widget">
                    ...
                    <div class="search-form-wrapper">
                        <select id="event-type-filter" class="form-select">
                            ...
                            <option value="temple" {{ request('type') == 'temple' ? 'selected' : '' }}>Temples</option>
                        </select>
                    </div>
                    <div class="search-input-wrapper mt-3">
                        <form id="event-search-form" class="search-form">
                            <input type="text" name="search" id="event-search-input" placeholder="Search Events" value="{{ request('search') }}" autocomplete="off">
                            <button type="submit">
                ...
                <div class="sidebar-widget">
                    ...
                    <div class="recent-event-item">
                        @if(isset($recentEvent->temple_id))
                            <a href="{{ route('frontend.event.details.unified', ['type' => 'temple', 'id' => $recentEvent->id]) }}">
                                <div class="recent-event-image">
                                    <img src="{{ $recentEvent->banner_image ? asset('backend/uploads/temple_event/banner/' . $recentEvent->banner_image) : 'https://placehold.co/150x150/a9561f/ffffff?text=Temple' }}" alt="{{ $recentEvent->event_name }}">
                                </div>
                                <div class="recent-event-content">
                                    ...
                                    <div class="recent-event-date">
                                        ...
                                        @if($recentEvent->event_date_end && $recentEvent->event_date != $recentEvent->event_date_end)
                                            {{ \Carbon\Carbon::parse($recentEvent->event_date)->format('M d') }} - {{ \Carbon\Carbon::parse($recentEvent->event_date_end)->format('M d, Y') }}
                                        @else
                        ...
                        @else
                            <a href="{{ route('frontend.event.details.unified', ['type' => 'organization', 'id' => $recentEvent->id]) }}">
                                <div class="recent-event-image">
                                    <img src="{{ $recentEvent->banner_image ? asset('backend/uploads/organization_event/banner/' . $recentEvent->banner_image) : 'https://placehold.co/150x150/c94641/ffffff?text=Org' }}" alt="{{ $recentEvent->event_name }}">
                                </div>
                                <div class="recent-event-content">
                                    <div class="recent-event-title">{{ Str::limit($recentEvent->event_name, 50) }}</div>
                                    <div class="recent-event-date">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function performEventSearch() {
            ...
            $.ajax({
                url: '{{ route('frontend.events') }}?' + params.toString(),
                method: 'GET',
                success: function(data) {
                    if (data.html) {
                        ...
                        let newUrl = '{{ route('frontend.events') }}' + (params.toString().replace('&ajax=1', '').replace('ajax=1&', '').replace('ajax=1', '') ? '?' + params.toString().replace('&ajax=1', '').replace('ajax=1&', '').replace('ajax=1', '') : '');
        ...
        if (showAllBtn) {
            showAllBtn.addEventListener('click', function() {
                ...
                fetch('{{ route('frontend.events') }}?show_all=1', {
                    method: 'GET',
                    headers: {
        ...
        function createEventHTML(event) {
            ...
            const imageUrl = isTemple
                ? (event.banner_image ? '{{ asset('backend/uploads/temple_event/banner/') }}/' + event.banner_image : 'https://placehold.co/150x150/a9561f/ffffff?text=Temple')
                : (event.banner_image ? '{{ asset('backend/uploads/organization_event/banner/') }}/' + event.banner_image : 'https://placehold.co/150x150/c94641/ffffff?text=Org');

            const detailUrl = isTemple
                ? '{{ url('/event/temple') }}/' + event.id
                : '{{ url('/event/organization') }}/' + event.id;
