...
@section('title', 'All Temples - Bengali Hindu Unity')
@section('stylesheet')
<style>
/* Temple Image Styles */
    .sigma_post-thumb {
    ...
    <div class="row">
        ...
        <div class="col-lg-3 d-none d-lg-block">
            <div class="sidebar">
                <div class="sidebar-widget widget-temple-filter" id="desktopFilterContainer">
                    ...
                    {{-- Form will be here on desktop via JS --}}
                </div>
            </div>
        </div>
        ...
        <div class="offcanvas offcanvas-start" tabindex="-1" id="filterDrawer" aria-labelledby="filterDrawerLabel">
            ...
            <div class="offcanvas-body" id="mobileFilterContainer">
                {{-- Form will be moved here on mobile via JS --}}
            </div>
        ...
        <template id="filterFormTemplate">
            <form id="templeFilterForm">
                ...
                <div class="mb-3">
                    ...
                    <select name="upazila_id" id="upazila" class="form-select">
                        ...
                        @foreach($upazilas as $upazila)
                            <option value="{{ $upazila->id }}" data-district="{{ $upazila->district_id }}">{{ $upazila->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3">
                    <label class="form-label">{{ $category->name }} @if($category->name_bn)({{ $category->name_bn }})@endif</label>
                    ...
                    <div class="form-check">
                        ...
                        <label class="form-check-label" for="activity_{{ $activity->id }}">
                            {{ $activity->title }} @if($activity->title_bn)({{ $activity->title_bn }})@endif
                        </label>
                    </div>
                    @endforeach
                </div>
        ...
        <div class="col-lg-9">
            ...
            <div class="row" id="temples-grid">
                @include('frontend.partials.temples-grid', ['temples' => $temples])
            </div>
            <!-- Pagination Start -->
            <div id="temples-pagination">
                @include('frontend.partials.temples-pagination', ['temples' => $temples])
            </div>
            <!-- Pagination End -->
        </div>
...
(function() {
    ...
    // ── State ────────────────────────────────────────────────────────────
    const temples = @json($templesData);
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function filterTemplesAjax(extraParams) {
            ...
            fetch('{{ route("frontend.temples.filter") }}?' + params, {
        ...
        (function($) {
            ...
            $searchInput.on('input', function() {
                ...
                searchTimer = setTimeout(function() {
                    ...
                    fetch('{{ route("api.temples.search") }}?query=' + encodeURIComponent(query), {
