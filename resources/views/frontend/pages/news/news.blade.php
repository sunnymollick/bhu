...
@section('title', 'News - Bengali Hindu Unity')
        ...
        <div class="breaking-news">
            ...
            <div class="breaking-news-ticker">
                <div class="breaking-news-content">
                    @foreach($newsList->take(5) as $index => $breakingNews)
                        {{ $breakingNews->title }}@if(!$loop->last) &nbsp;&nbsp;&nbsp;•&nbsp;&nbsp;&nbsp; @endif
                    @endforeach
                </div>
            </div>
        </div>
            ...
            <div class="col-lg-4">
                <div class="sidebar">
                    ...
                    <div class="sidebar-widget widget-search">
                        ...
                        <form method="GET" action="{{ route('frontend.news') }}" id="news-search-form">
                            <div class="sigma_search-adv-input">
                                <input type="text" class="form-control" id="news-search-input" placeholder="Search News..." name="search" value="{{ request('search') }}" autocomplete="off">
                    ...
                    <div class="sidebar-widget popular-tags-widget">
                        ...
                        <div class="tagcloud">
                            @if($popularLocations && $popularLocations->count() > 0)
                                @php
                                ...
                                @foreach($uniqueLocations->take(10) as $location)
                                    @php
                                    ...
                                    @endphp
                                    <a href="#" class="location-tag {{ $requestLocation == $trimmedLocation ? 'active' : '' }}" data-location="{{ $trimmedLocation }}">
                                        <i class="fas fa-map-marker-alt"></i>
                                        {{ $trimmedLocation }}
                                        <span class="tag-count">{{ $location->total }}</span>
                                    </a>
                                @endforeach

                                <a href="#" class="clear-filter-btn {{ request('location') ? 'show' : '' }}" id="clearFilter">
                                    <i class="fas fa-times-circle"></i>
                                    Clear Filter
                                </a>

                                @if($uniqueLocations->count() > 10)
                                    <a href="#" class="show-all-tags" id="showAllTags">
                                        <i class="fas fa-tags"></i>
                                        Show All ({{ $uniqueLocations->count() }})
                                    </a>
                                @endif
                            @else
                        ...
                        <article class="sigma_post">
                            <div class="sigma_post-thumb">
                                @if($news->attachments && count($news->attachments) > 0)
                                    <a href="{{ route('frontend.news.details', $news->id) }}">
                                        <img src="{{ asset($news->attachments[0]) }}" alt="{{ $news->title }}">
                                    </a>
                                @else
                                    <a href="{{ route('frontend.news.details', $news->id) }}">
                                        <img src="https://via.placeholder.com/400x280/7E4555/ffffff?text=News+Image" alt="{{ $news->title }}">
                                    </a>
                                @endif
                                ...
                                <span class="news-category-badge">
                                    <i class="fas fa-map-marker-alt"></i> {{ $news->location }}
                                </span>
                                @endif

                                <!-- Read More Button -->
                                <a href="{{ route('frontend.news.details', $news->id) }}" class="read-more-overlay">
                            ...
                            <div class="sigma_post-body">
                                <div class="sigma_post-meta">
                                    <div>
                                        ...
                                        <a href="{{ route('frontend.news.details', $news->id) }}" class="sigma_post-date">{{ $news->date_time->format('M d, Y') }}</a>
                                    </div>
                                </div>
                                <h5>
                                    <a href="{{ route('frontend.news.details', $news->id) }}">{{ $news->title }}</a>
                                </h5>
                                <p class="news-excerpt">{{ Str::limit(strip_tags($news->final_news ?? $news->short_news), 120) }}</p>
                                <div class="sigma_post-single-author">
                                    <a href="{{ route('frontend.news.details', $news->id) }}" class="read-more-btn">
                ...
                <div class="row">
                    <div class="col-12">
                        <ul class="pagination">
                            ...
                            @else
                                <li class="page-item"><a class="page-link" href="{{ $newsList->previousPageUrl() }}" rel="prev">‹</a></li>
                            @endif
                            ...
                            @if ($newsList->hasMorePages())
                                <li class="page-item"><a class="page-link" href="{{ $newsList->nextPageUrl() }}" rel="next">›</a></li>
                            @else
                                <li class="page-item disabled"><span class="page-link">›</span></li>
                            @endif
                        </ul>
...
$(document).ready(function() {
    ...
    function performSearch(query, location = '') {
        const url = new URL('{{ route("frontend.news") }}');
        if (query) {
