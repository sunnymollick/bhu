    ...
    <div class="container-fluid">
        ...
        <div class="career-section-card">
            ...
            <div id="objectiveContent">
                @if($career->career_objective)
                    <p id="objectiveText" class="text-truncated">{{ $career->career_objective }}</p>
                    @if(strlen($career->career_objective) > 200)
            ...
            <div class="collapse mt-3" id="objectiveForm">
                <textarea id="careerObjectiveText" class="form-control" rows="4" placeholder="Enter your career objective...">{{ $career->career_objective }}</textarea>
                <div class="mt-3">
...
<div class="career-section-card">
    ...
    <div id="educationList">
        @if($career->education && count($career->education) > 0)
            @foreach($career->education as $edu)
                <div class="item-card" data-id="{{ $edu['id'] }}">
                    <div class="item-header">
                        <div>
                            ...
                            <div class="item-meta">
                                <span><i class="fas fa-calendar mr-1"></i>{{ date('M Y', strtotime($edu['start_date'])) }} - {{ $edu['end_date'] ? date('M Y', strtotime($edu['end_date'])) : 'Present' }}</span>
                                @if(!empty($edu['location']))
                                    <span><i class="fas fa-map-marker-alt mr-1"></i>{{ $edu['location'] }}</span>
                                @endif
                                @if(!empty($edu['cgpa']))
                                    <span><i class="fas fa-star mr-1"></i>CGPA: {{ $edu['cgpa'] }}</span>
                                @endif
                                @if(!empty($edu['group']))
                                    <span><i class="fas fa-book mr-1"></i>{{ $edu['group'] }}</span>
                                @endif
                            </div>
                        </div>
                    ...
                    @if(!empty($edu['major']) || !empty($edu['minor']))
                        <p class="mb-0 mt-2 text-muted">
                            @if(!empty($edu['major']))Major: {{ $edu['major'] }}@endif
                            @if(!empty($edu['major']) && !empty($edu['minor'])) | @endif
                            @if(!empty($edu['minor']))Minor: {{ $edu['minor'] }}@endif
                        </p>
                    @endif
                </div>
            @endforeach
...
<div class="career-section-card">
    ...
    <div id="experienceList">
        @if($career->work_experience && count($career->work_experience) > 0)
            @foreach($career->work_experience as $exp)
                <div class="item-card" data-id="{{ $exp['id'] }}">
                    <div class="item-header">
                        <div>
                            ...
                            <div class="item-meta">
                                <span><i class="fas fa-calendar mr-1"></i>{{ date('M Y', strtotime($exp['start_date'])) }} - {{ $exp['end_date'] ? date('M Y', strtotime($exp['end_date'])) : 'Present' }}</span>
                    ...
                    @if(!empty($exp['description']))
                        <div class="description-container">
                            <div id="description-{{ $exp['id'] }}" class="text-truncated">{!! $exp['description'] !!}</div>
                            @if(strlen(strip_tags($exp['description'])) > 200)
                                <button class="see-more-btn" id="toggle-{{ $exp['id'] }}" onclick="toggleDescription('{{ $exp['id'] }}')" >See More</button>
                            @endif
                        </div>
                    @endif
                </div>
...
<div class="career-section-card">
    ...
    <div id="projectsList">
        @if($career->projects && count($career->projects) > 0)
                ...
                <div class="item-card" data-id="{{ $project['id'] }}">
                    <div class="item-header">
                        <div>
                            ...
                            <div class="item-meta">
                                <span><i class="fas fa-calendar mr-1"></i>{{ date('M Y', strtotime($project['start_date'])) }} - {{ $project['end_date'] ? date('M Y', strtotime($project['end_date'])) : 'Present' }}</span>
                                @if(!empty($project['link']))
                                    <span><i class="fas fa-link mr-1"></i><a href="{{ $project['link'] }}" target="_blank">View Project</a></span>
                                @endif
                            </div>
                        </div>
                        <div class="item-actions d-flex gap-2">
...
window.educationData = @json($career->education ?? []);
window.experienceData = @json($career->work_experience ?? []);
window.skillsData = @json($career->skills ?? []);
window.projectsData = @json($career->projects ?? []);
window.certificationsData = @json($career->certifications ?? []);
window.languagesData = @json($career->languages ?? []);
window.professionalLinksData = @json($career->professional_links ?? []);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (saveObjectiveBtn) {
        saveObjectiveBtn.addEventListener('click', function() {
            ...
            // Send AJAX request
            fetch('{{ route("admin.career.update.objective") }}', {
                method: 'POST',
                headers: {
    ...
    if (saveEducationBtn) {
        saveEducationBtn.addEventListener('click', function() {
            ...
            const url = educationId
                ? '{{ route("admin.career.education.update") }}'
                : '{{ route("admin.career.education.add") }}';
...
document.getElementById('confirmDeleteEducationBtn').addEventListener('click', function() {
    ...
    fetch('{{ route("admin.career.education.delete") }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
document.getElementById('saveExperienceBtn').addEventListener('click', function() {
    ...
    const url = experienceId
        ? '{{ route("admin.career.experience.update") }}'
        : '{{ route("admin.career.experience.add") }}';
...
document.getElementById('confirmDeleteExperienceBtn').addEventListener('click', function() {
    ...
    fetch('{{ route("admin.career.experience.delete") }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
document.getElementById('saveSkillBtn').addEventListener('click', function() {
    ...
    const url = skillId
        ? '{{ route("admin.career.skill.update") }}'
        : '{{ route("admin.career.skill.add") }}';
...
document.getElementById('confirmDeleteSkillBtn').addEventListener('click', function() {
    ...
    fetch('{{ route("admin.career.skill.delete") }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
document.getElementById('saveProjectBtn').addEventListener('click', function() {
    ...
    const url = projectId
        ? '{{ route("admin.career.project.update") }}'
        : '{{ route("admin.career.project.add") }}';
...
document.getElementById('confirmDeleteProjectBtn').addEventListener('click', function() {
    ...
    fetch('{{ route("admin.career.project.delete") }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
document.getElementById('saveCertificationBtn').addEventListener('click', function() {
    ...
    const url = certificationId
        ? '{{ route("admin.career.certification.update") }}'
        : '{{ route("admin.career.certification.add") }}';
...
document.getElementById('confirmDeleteCertificationBtn').addEventListener('click', function() {
    ...
    fetch('{{ route("admin.career.certification.delete") }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
document.getElementById('saveLanguageBtn').addEventListener('click', function() {
    ...
    const url = languageId
        ? '{{ route("admin.career.language.update") }}'
        : '{{ route("admin.career.language.add") }}';
...
document.getElementById('confirmDeleteLanguageBtn').addEventListener('click', function() {
    ...
    fetch('{{ route("admin.career.language.delete") }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
if (saveSocialLinkBtn) {
    ...
    const endpoint = isEditing ? '{{ route("admin.career.link.update") }}' : '{{ route("admin.career.link.add") }}';
    const requestData = {
        platform: platform,
        url: url,
        icon: icon
...
document.getElementById('confirmDeleteProfessionalLinkBtn').addEventListener('click', function() {
    ...
    fetch('{{ route("admin.career.link.delete") }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
